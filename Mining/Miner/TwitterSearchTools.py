...
import Miner.TwitterLogin
from Mining.Exceptions.MiningErrorClasses import *
from Mining.SearchResultsProcessing import TweetResults
...
class Search:
    ...
    def __init__( self ):
        ...
        self._observers = [ ]
        self.tweets = [ ]
        self.tweet_factory = TweetResults.TweetFactory()
    ...
    def set_twitter_connection( self, credentials ):
        """
        Args:
            credentials: String location of twitter credentials file
            login: TwitterLogin.login
        """
        self.twitter_conn = Miner.TwitterLogin.login( credentials )
    ...
    def run( self, list_of_search_terms, limit, recent=True, rest=800 ):
        """
        Performs the search.
        This version internalizes the timeout functions so that it only grabs the newest tweet
        once per search term list
        ...
        Args:
            ...
            recent: Boolean True search for tweets newer than most recent record;
                False search for tweets older than oldest record
